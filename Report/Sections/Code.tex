...
were implemented. Therefore, to perform copy operations, one needs to use mainly \code{MOV} from and towards \code{B}.
...
The idea is to fetch from memory a stored value of a \code{COUNTER} (which can be freely initialized), execute "loop" code
and followingly increase the counter and perform a comparison with an \code{UPPER\_LIMIT}.
...
The code executed in the loop fetches an initial address \code{NUMBERS\_ADDR} and uses it to write the current cpunter value to the address
